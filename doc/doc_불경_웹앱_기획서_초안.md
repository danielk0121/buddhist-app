...
서비스가 반드시 제공해야 하는 핵심 화면은 다음 4가지다.

- **경전 수필 목록 보기** — 45종 경전을 계열별·인기순으로 탐색하는 목록 화면
- **경전 수필 상세 보기** — 단락별 본문, 한자 해설, TTS, 댓글을 제공하는 상세 화면
- **개인 설정 화면** — 테마, 폰트 크기, TTS 속도 등 사용자 커스텀 설정 화면
- **개발자 정보 화면** — 앱 버전, 개발자 정보, 저작권 안내, 오픈소스 고지 화면 링크 포함
...
모든 화면은 **상단 툴바(Top App Bar)** 와 **탭바(Tab Bar)** 를 공통으로 사용한다.

**상단 툴바**
- 좌측: 뒤로가기 버튼 (상세·서브 화면) 또는 앱 로고
- 중앙: 현재 화면 제목
- 우측: 화면별 액션 버튼 (검색, 설정 진입 등)

**탭바 (툴바 바로 아래 고정)**
- 경전 탭 — 홈(인기 TOP5 + 계열별), 경전 목록, 경전 상세
- 오디오북 탭 — TTS 재생 현황, 바로 듣기 목록, TTS 안내
- 설정 탭 — 테마, 폰트 크기, TTS 속도, 개발자 정보 링크

> 경전 상세(SutraDetail), 개발자 정보(About) 화면은 탭바를 숨기고 전체 화면으로 표시한다.
...
**경전 수필 목록 화면**
- 상단 툴바: "경전 목록" 제목 + 검색 버튼
- 계열별 탭 필터
- 경전 카드: 제목(한글) + 한자 + 짧은 소개 + 조회수/댓글수

**경전 수필 상세 화면**
- 상단 툴바: 뒤로가기 + 경전명 + 북마크 버튼
- TTS 플레이어 고정 하단바
...
**개발자 정보 화면**
- 상단 툴바: 뒤로가기 + "개발자 정보" 제목
- 앱 이름 및 버전
- 개발자 이름 / 이메일
- 저작권 안내 (콘텐츠 출처, 원전 공유 저작물 고지)
- 오픈소스 고지 화면 링크 (사용 라이브러리 목록)
- GitHub 저장소 링크
...
### 6.4 더미 API 인터페이스 (추후 Supabase 교체 대상)
...
- [x] 마크다운 → JSON 변환 스크립트 작성
- [x] SQLite(sql.js) 초기화 및 데이터 로딩
- [x] 홈, 경전 목록, 경전 상세 화면 구현
- [x] 상단 툴바 + 탭바 공통 레이아웃 적용
- [x] 홈 화면 — 세로형 카드 타일 그리드 (3열) 구현
- [ ] 한자 훈음 인라인 툴팁 UI
- [ ] 조회수 카운트 (로컬 SQLite)
...
- [x] 단락 하이라이트 동기화
- [x] 재생 속도 조절
- [x] 오디오북 탭 — 웹앱 메인 메뉴 탭바에 추가
- [x] 오디오북 페이지 구현 (현재 재생 배너, 바로 듣기 목록, TTS 안내)
